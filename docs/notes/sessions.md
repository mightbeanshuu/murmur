...
## Session 25 — PostgreSQL in Murmur (the untaught third data store)
- **Date:** 30 July 2026.
...
- **Murmur Mapping:** current single coarse Activity = durable dispatch without step-level resume; `workflowId: swarm-<runId>` as free dedup; `maximumAttempts: 1` as a deliberate non-idempotency decision; `cancellationSignal()` bridging into the Session-16 AbortSignal plumbing; `localDelivery:false` + Redis polling as the cross-process SSE bridge; the concrete phase-level Activity redesign that would unlock retries.
